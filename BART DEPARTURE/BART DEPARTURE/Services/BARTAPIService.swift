...
struct BARTAPIService {
    ...
    struct BARTRoot: Codable {
        ...
        // Make all fields optional to handle different response types
        enum CodingKeys: String, CodingKey {
            case station, stations, message, uri
        }
        
        init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            station = try? container.decodeIfPresent([BARTStation].self, forKey: .station)
            stations = try? container.decodeIfPresent(BARTStations.self, forKey: .stations)
            message = try? container.decodeIfPresent(BARTMessage.self, forKey: .message)
            uri = try? container.decodeIfPresent(BARTURI.self, forKey: .uri)
        }
    ...
    func fetchDepartures(for stationAbbr: String) async throws -> [BARTETD] {
        ...
        guard httpResponse.statusCode == 200 else {
            let statusError = "HTTP \(httpResponse.statusCode): \(HTTPURLResponse.localizedString(forStatusCode: httpResponse.statusCode))"
            throw BARTAPIError.apiError(statusError)
        }
        
        // Check if response is actually JSON
        if let responseString = String(data: data, encoding: .utf8) {
            if responseString.trimmingCharacters(in: .whitespaces).hasPrefix("<") {
                throw BARTAPIError.apiError("BART API returned XML instead of JSON. Make sure 'json=y' parameter is included.")
            }
            print("BART ETD Response for \(stationAbbr) (first 500 chars): \(String(responseString.prefix(500)))")
        }
        ...
        do {
            ...
            let errorDescription = "JSON decoding failed for station \(stationAbbr): \(decodingError.localizedDescription)"
            print(errorDescription)
            if let responseString = String(data: data, encoding: .utf8) {
                print("Full response: \(responseString)")
            }
            throw BARTAPIError.apiError(errorDescription)
        } catch {
            throw BARTAPIError.apiError("Failed to parse BART API response: \(error.localizedDescription)")
        }
    }
    ...
    func fetchAllStations() async throws -> [BARTStationInfo] {
        ...
        if let responseString = String(data: data, encoding: .utf8) {
            ...
            print("BART API Response (first 500 chars): \(String(responseString.prefix(500)))")
        }
        ...
        do {
            ...
            let errorDescription = "JSON decoding failed: \(decodingError.localizedDescription)"
            print(errorDescription)
            if let responseString = String(data: data, encoding: .utf8) {
                print("Full response: \(responseString)")
            }
