...
struct BARTDepartureProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<BARTDepartureEntry>) -> Void) {
        ...
        // Note: Widgets can access location if the main app has permission
        let manager = locationManager
        guard let location = manager.location else {
            // Try to get location one more time
            // In widgets, location might not be immediately available
            let entry = BARTDepartureEntry(
                date: currentDate,
                station: nil,
                error: "Location unavailable. Make sure location services are enabled.",
                isLocationAuthorized: true
            )
            // Retry in 30 seconds
            let timeline = Timeline(entries: [entry], policy: .after(currentDate.addingTimeInterval(30)))
            completion(timeline)
            return
        }
...
// MARK: - Widget View
struct BARTDepartureWidgetView: View {
...
struct BARTDepartureWidget: Widget {
    let kind: String = "BARTDepartureWidget"
    
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: BARTDepartureProvider()) { entry in
            BARTDepartureWidgetView(entry: entry)
...
// MARK: - Widget Bundle
// NOTE: Remove the @main attribute below if this widget is in the same target as your main app.
// The @main should only be present when the widget is in a separate Widget Extension target.
